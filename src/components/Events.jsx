...
import { Terminal, Shield, Cpu, Zap, Trophy, Clock, ArrowRight, X, FileText, UploadCloud } from 'lucide-react';
...
const Events = () => {
  ...
  useEffect(() => {
    // Check if redirected to open Web3 event modal (e.g. returning from web3 pages)
    const params = new URLSearchParams(window.location.search);
    const eventParam = params.get('event');
    if (eventParam === 'web3' || eventParam === 'ai-arena') {
      const web3Event = eventsData.find((e) => e.id === 'ai-arena');
      if (web3Event) {
        setActiveModalEvent(web3Event);
  ...
  // Lock background scrolling and prevent Lenis smooth scroll while modal is active
  useEffect(() => {
    if (activeModalEvent) {
      window.__lenis?.stop();
  ...
  const handleSubmitPPT = () => {
    playHeistClickSound();
    const targetUrl =
      WEB3_CONFIG.pptSubmissionUrl ||
      'https://drive.google.com/drive/folders/1aSvKpK6QFAVFk2E8VLvKbumSY8Ax6TCN?usp=drive_link';
...
{activeModalEvent && (
  <div
    ...
    className="fixed inset-0 z-[9999] bg-black/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 overflow-hidden overscroll-contain"
    onClick={closeModal}
  >
    <div
      data-lenis-prevent="true"
      ...
      className="relative w-full max-w-2xl bg-zinc-950 border border-red-600/60 rounded-2xl p-6 sm:p-8 shadow-[0_0_50px_rgba(230,0,0,0.4)] text-white max-h-[85vh] sm:max-h-[88vh] overflow-y-auto overscroll-contain my-auto"
    >
      ...
      <button
        ...
        className="sticky top-0 float-right z-30 w-10 h-10 -mr-2 -mt-2 sm:-mr-4 sm:-mt-4 flex items-center justify-center text-gray-300 hover:text-white bg-zinc-900/90 hover:bg-red-600 border border-zinc-700 hover:border-red-500 rounded-full cursor-pointer transition-all duration-200 shadow-lg backdrop-blur-sm"
      ...
      {/* Web3 Exclusive Resource Actions: Problem Statement & PPT Submission */}
      {activeModalEvent.id === 'ai-arena' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
          <button
            type="button"
            onClick={(event) => {
              event.stopPropagation();
              handleViewProblemStatement();
            }}
            className="w-full py-3 px-4 bg-zinc-900/90 hover:bg-zinc-800/95 border border-red-600/50 hover:border-red-500 text-gray-200 hover:text-white font-bebas text-xl tracking-wider rounded-xl transition-all duration-200 shadow-[0_0_15px_rgba(230,0,0,0.15)] hover:shadow-[0_0_20px_rgba(230,0,0,0.35)] flex items-center justify-center gap-2 cursor-pointer"
          >
          ...
          <button
            ...
            className="w-full py-3 px-4 bg-zinc-900/90 hover:bg-zinc-800/95 border border-red-600/50 hover:border-red-500 text-gray-200 hover:text-white font-bebas text-xl tracking-wider rounded-xl transition-all duration-200 shadow-[0_0_15px_rgba(230,0,0,0.15)] hover:shadow-[0_0_20px_rgba(230,0,0,0.35)] flex items-center justify-center gap-2 cursor-pointer"
          >
            <UploadCloud size={18} className="text-red-500 shrink-0" />
