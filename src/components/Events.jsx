...
import hackerHeistImg from '../../EVENT_IMAGES/hacker-heist.png';
import web3Img from '../../EVENT_IMAGES/web3--.jpeg';
import detectyxImg from '../../EVENT_IMAGES/detectyx--.jpeg';
import campusRushImg from '../../EVENT_IMAGES/campus-rush.jpeg';

const eventsData = [
  {
    id: 'cyber-hack',
    title: "HACKER'S HEIST",
    category: 'CTF Event',
    icon: Terminal,
    image: hackerHeistImg,
...
const Events = () => {
  ...
  const [modalTransitionText, setModalTransitionText] = useState('');
  ...
  const handleRegisterClick = (eventId) => {
    playHeistClickSound();
    setModalTransitionText('REGISTRATION REQUEST RECEIVED');

    setTimeout(() => {
      setModalTransitionText('VERIFYING CLEARANCE...');
    }, 180);

    setTimeout(() => {
      setModalTransitionText('ACCESS GRANTED // REDIRECTING');
    }, 360);
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-4 flex-wrap mb-12">
          {categories.map((cat) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full font-bebas text-lg tracking-wider transition-all duration-300 cursor-pointer ${selectedCategory === cat
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredEvents.map((evt) => {
            ...
            return (
              <div
                key={evt.id}
                className="group relative bg-zinc-950/90 border border-zinc-800/80 hover:border-red-600/80 rounded-2xl p-5 transition-all duration-300 flex flex-col justify-between shadow-[0_0_20px_rgba(0,0,0,0.6)] hover:shadow-[0_0_35px_rgba(230,0,0,0.25)] hover:-translate-y-1.5"
              >
                <div className="flex flex-col">
                  ...
                  <div className="relative w-full aspect-[4/5] rounded-xl overflow-hidden border border-zinc-800/90 group-hover:border-red-500/40 mb-4 bg-zinc-900/60 transition-colors duration-300">
                    <img
                      src={evt.image}
                      alt={evt.title}
                      ...
                      loading="lazy"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-zinc-950/80 via-transparent to-transparent pointer-events-none opacity-60 group-hover:opacity-30 transition-opacity duration-300" />
                  </div>

                  {/* Event Title */}
                  <h3 className="font-bebas text-2xl sm:text-3xl tracking-wider text-white mb-4 group-hover:text-red-500 transition-colors">
                    {evt.title}
                  </h3>
                </div>
                ...
                <div>
                  {/* Meta Details */}
                  <div className="flex items-center justify-between text-xs font-mono-cyber text-gray-400 border-t border-zinc-800/80 pt-4 mb-4">
          ...
          <div className="relative w-full max-w-2xl bg-zinc-950 border border-red-600/60 rounded-2xl p-6 sm:p-8 shadow-[0_0_50px_rgba(230,0,0,0.4)] text-white max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-6 mb-6">
              {activeModalEvent.image && (
                <div className="w-full sm:w-48 aspect-[4/5] rounded-xl overflow-hidden border border-zinc-800 bg-zinc-900 shrink-0">
                  <img
              ...
              <div className="flex-1 flex flex-col justify-between">
                ...
                <div className="grid grid-cols-2 gap-4 bg-zinc-900/80 p-4 rounded-xl border border-zinc-800 font-mono-cyber text-xs">
                  <div>
            ...
            <button
              ...
              disabled={!!modalTransitionText}
              className={`w-full py-4 bg-red-600 hover:bg-red-700 text-white font-bebas text-2xl tracking-widest rounded-xl transition-all duration-300 shadow-[0_0_25px_rgba(230,0,0,0.6)] cursor-pointer flex items-center justify-center gap-2 ${
                modalTransitionText ? "bg-red-700 opacity-90 cursor-wait" : ""
              }`}
            >
              {modalTransitionText ? (
                <span className="font-mono-cyber text-base tracking-widest text-white animate-pulse">
                  {modalTransitionText}
                </span>
              ) : (
