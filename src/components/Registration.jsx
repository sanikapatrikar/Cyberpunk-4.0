...
function Registration() {
  ...
  const updateParticipant = (index, key, value) => {
    ...
    setFieldErrors((prev) => {
      const next = { ...prev };
      delete next[`participant-${index}-${key}`];
      return next;
    });
  };
  ...
  const validateParticipants = () => {
    ...
    if (!form.teamName?.trim()) {
      errors.teamName = "Team / crew codename is required.";
    }
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!participant.fullName?.trim()) {
        errors[`participant-${i}-fullName`] =
          `Enter the full name of operative ${i + 1}.`;
      }

      if (!validateEmail(participant.email || "")) {
        errors[`participant-${i}-email`] =
          `Enter a valid email for operative ${i + 1}.`;
      }

      if (!validatePhone(participant.phone || "")) {
        errors[`participant-${i}-phone`] =
          `Enter a valid phone number for operative ${i + 1}.`;
      }

      if (!participant.college?.trim()) {
        errors[`participant-${i}-college`] =
          `Enter the college of operative ${i + 1}.`;
      }

      if (!participant.branch) {
        errors[`participant-${i}-branch`] =
          `Select the branch of operative ${i + 1}.`;
      }

      if (!participant.year) {
        errors[`participant-${i}-year`] =
          `Select the year of operative ${i + 1}.`;
      }
    }
                    ...
                    >
                      ...
                      <div className="cp-form-grid">
                        <Field
                          ...
                          error={fieldErrors[`participant-${index}-fullName`]}
                        />
                        ...
                        <Field
                          ...
                          error={fieldErrors[`participant-${index}-email`]}
                        />
                        ...
                        <Field
                          ...
                          error={fieldErrors[`participant-${index}-phone`]}
                        />
                        ...
                        <Field
                          ...
                          error={fieldErrors[`participant-${index}-college`]}
                        />
                        ...
                        <SelectField
                          ...
                          error={fieldErrors[`participant-${index}-branch`]}
                        />
                        ...
                        <SelectField
                          ...
                          error={fieldErrors[`participant-${index}-year`]}
                        />
                      </div>
                    </div>
                  )
