  ...
  const submitRegistration = async () => {
    ...
    try {
      /* --------------------------------
         FINAL VALIDATION
      -------------------------------- */
      if (!selectedEvent) throw new Error("Event information is missing.");
      if (!teamConfig) throw new Error("Team configuration is missing.");
      if (!form.teamName.trim()) throw new Error("Team / crew codename is required.");
      if (!form.transactionId.trim()) throw new Error("Transaction ID / UTR is required.");
      if (!form.paymentScreenshot) throw new Error("Payment screenshot is required.");

      const participantError = validateParticipants();
      if (participantError) throw new Error(participantError);

      /* --------------------------------
         USE THE SCREENSHOT PREPARED WHILE
         THE USER WAS FILLING THE FORM
      -------------------------------- */
      ...
      /* Vercel Functions have a request payload limit. Keep the final
         encoded screenshot comfortably below that limit. */
      if (screenshotBase64 && screenshotBase64.length > 3_500_000) {
        throw new Error(
          "Payment screenshot is too large after processing. Please upload a smaller image/PDF."
        );
      }

      const cleanParticipants = form.participants
        .slice(0, participantCount)
        .map((participant) => ({
          fullName: participant.fullName.trim(),
          email: participant.email.trim(),
          phone: participant.phone.replace(/\s+/g, "").trim(),
      ...
      /* --------------------------------
         NEW FAST API
         Same-origin Vercel server function.
         The UI moves to CLEAR only after the
         server confirms that the registration
      ...
      if (!response.ok || !result?.success) {
        if (result?.duplicate) {
          throw new Error(
            result.error || "This transaction ID / UTR has already been used."
          );
        }
        throw new Error(result?.error || "Registration could not be saved.");
      }

      /* IMPORTANT: success is shown only after the API confirms the
         Google Sheet append completed successfully. */
      setStep(5);
